    ...
    <div class="row">
      <div class="col-md-12">
        <div class="block text-center">
          <span class="text-white">Book your Consultation</span>
    ...
    <div class="row">
      ...
      <div class="col-lg-8">
           <div class="appoinment-wrap mt-5 mt-lg-0 pl-lg-5">
            <h2 class="mb-2 title-color">Book an Consulattion</h2>
            <p class="mb-4">Mollitia dicta commodi est recusandae iste, natus eum asperiores corrupti qui velit . Iste dolorum atque similique praesentium soluta.</p>
            {!! Form::open(['class' =>'form-sample','route' => 'make-consultation.store','method' => 'POST', 'enctype' => 'multipart/form-data']) !!}
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                <select class="form-control response" id="doctor_name" name="doctor_name" data-response="doctor">
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                <select class="form-control response" id="schedule" name="schedule">
                    ...
                    <button type="submit" class="btn btn-main btn-round-full">Book Consultation Now<i class="icofont-simple-right ml-2"></i></button>
                {!! Form::close() !!}
            </div>
        </div>
      </div>
...
<script>
  ...
  $(document).on('change','.response',function(){
      ...
      {
        $.ajax({
              url : '/make-consultation-department-doctor-schedule-find/' + value,
              method : 'GET',
              success : function(data){
                  console.log(data)
                  if(data.length === 0){
                      toastr.error("Schedule Does Not Exist");
                      $('#schedule').empty().append('<option selected="" value="" disabled selected hidden style="select:invalid { color: gray; }" readonly="" data-live-search="true">Choose a Schedule</option>');
                      // refresh();
                  }else{
                      $('#schedule').empty().append('<option selected="" value="" disabled selected hidden style="select:invalid { color: gray; }" readonly="" data-live-search="true">Choose a Schedule</option>');
                      let size=data['day'].length
                    //   jQuery.each( data, function( item, element ) {
                
                          for(let i=0; i <size  ; i++)
                          {
                            $('#schedule').append(`<option value='${i}'>${data['day'][i]} : ${data['start_time'][i]} - ${data['end_time'][i]} </option>`);
                          }
                        //   $('#schedule').append("<option value='" + data['id'] + "'>"+ data['name']+"</option>");
                        //   });
                      $('#id_schedule').append(`<input name="schedule_id" id="schedule_id" type="hidden" value=" ${data['id']}" class="form-control">`)
                      // refresh();
                  }
              }
          });
